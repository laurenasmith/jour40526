...
A few of the categories are very general such as "homicide," and "assault," but the categories also break certain crimes down to specific variations, such as assault with weapon, theft from auto or robbery residential. I would say these more specific categorizations make reporting easier. Broad categories like "assault" and "homicide" can possibly make data summarization difficult or even misleading. For example, homicide can include vehicular homicide, negligent homicide, manslaughter and other various charges that are different crimes.
...
```{r}
pg_crime |> mutate(total_incidents= (Jan + Feb + Mar + Apr + Jun + Jul + Aug + Sep+ Oct+ Nov+ Dec))

pg_crime_averages <- pg_crime |> mutate(average_incidents = (Jan + Feb + Mar + Apr + Jun + Jul + Aug + Sep+ Oct+ Nov+ Dec)/12)
```
...
The crime category with the highest average is Accident, while the category with the lowest average is Homicide. Other categories with lower averages are B&E Other, Assault (shooting and weapon), Sex Offense. Categories with higher averages are Auto Stolen and Theft From Auto. This suggests that average rates of violent crime in Prince George's County are relatively low, while theft seems to be the biggest issue.

One thing I noticed is that though Assault Shooting and With Weapon have low averages, the general Assault category has a higher average. This makes me wonder what other variations of assault are not specified.
...
summer_crime_averages <-
  pg_crime_averages |> mutate(june_change = (Jun - average_incidents)/average_incidents * 100)

  pg_crime_averages |> mutate(julychange = (Jul - average_incidents)/average_incidents * 100)

  pg_crime_averages |> mutate(august_change = (Aug - average_incidents)/average_incidents * 100)
```
```{r}
summer_crime_averages <-
   pg_crime_averages |> mutate(julychange = (Jul - average_incidents)/average_incidents * 100)
...
For each of the summer months, it is clear that while some crimes do have higher rates, they are most likely coincidental, as other crimes have lower rates during these months. There is also no distinction between violent and nonviolent crimes, meaning neither kind is more or less likely during the summer months. There are no particular patterns The data shows that the theory of higher crime in the summer months is likely inaccurate.
