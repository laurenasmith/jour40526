...
Income and bachelors degree or higher appear to have a strong positive correlation, same as with percentage of white people and turnout. Unemployed and turnout have a strong negative correlation. Median age has no correlation with bachelor's degree or higher.

The unemployed percentage and white percentage seem to have a linear relationship with turnout. Income does not appear to have a linear relationship with turnout.
...
The p-value shows that the model is statistically significant, and the residual standard error is low, showing that the model is pretty accurate in terms of voter turnout. The multiple r-squared tells us that around 85% of the variability in the data is explained by the model and the adjusted r-squared shows that even after accounting for the different variables the model is an 80% fit, which is better than any of the models from our class assignment. Thus, I feel that this is a useful model to predict voter turnout.

Most of the variables do seem to be pretty insignificant. The only impactful variable appears to be unemployment, which has a strong negative relationship with voter turnout. The next closest variable is pct_white, which has a coefficient of 0.1, which is close to zero, so we can gather that it is not very impactful.
...
The model I tried that comes closest to the full model is one that includes all of the variables except pct_unemployed. Because it seems that pct_unemployed is the most significant variable, I thought combining it with some of the other variables would produce a more precise model, but it seems that the full model is the most precise, at least from what I tried.
...
# Model 3
model3 <- lm(turnout ~ pct_unemployed + population + income, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ population + income + pct_white + pct_bachelors_higher + median_age, data = md_turnout)
summary(model4)
...
This model could be helpful for covering elections on a local level, because could examine the demographics of a the city or general area that is your beat for example, and then use this model to help predict how many people in that area will turn out to vote for an upcoming election. This information also could be used for a story about which demographic factors are correlated with lower turnout, and talk to people to explore different reasons why people belonging to certain demographics do not vote. You can also analyze this data by seeing which independent variables correlate with each other, and use multiple versions of this model to compare one voting population to another. For example, if two counties have a high percentage of unemployment but one has a higher white population, if their election turnouts were significantly different, a story could be written on why one county had a higher turnout despite the fact that they have a large demographic similarity.

I suppose this model could also be used retrospectively to cover if a voting district exceeded turnout expectations set by the model in a past election. You can also analyze this data by seeing which independent variables correlate with each other, and use multiple versions of this model to compare one voting population to another. For example, if two counties have a high percentage of unemployment but one has a higher white population, if their election turnouts were significantly different, a story could be written on 
