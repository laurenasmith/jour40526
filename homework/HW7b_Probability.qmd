...
The shape of the distribution is very abnormal. The data is positively skewed to the right. Most of the salaries are clustered around roughly 75000.
...
The data follows the empirical rule relatively closely (within 5% of normal distribution). The distribution is abnormal, but the actual percentages reflect that it is also narrower than what a normal distribution would be.
...
The probability estimate is that roughly 16% of White House employees have a salary over $150,000. This estimate is slightly lower than the actual proportion of around 18%.
...
Same as before, the probability estimate and actual proportion are relatively close. This middle range demonstrates that a good portion of White House salaries fall within these two values, but not the majority of the data.
...
The biggest difference is in the 60k-90k salary range. There is a much higher count of salaries in this range than a normal distribution would expect. As demonstrated earlier in the histogram, the data is mostly clustered close to 75k. It is clear that White House salaries are not normally distributed and that there are certain employees that are getting paid much higher than most.
