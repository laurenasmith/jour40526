...
I would say this is a somewhat useful model, but it could definitely be improved. The p-values for each of the variables show that only one has any statistically significant impact on wins, and the coefficient shows that impact to be small. All of the coefficients are pretty low, which means that the relationship between the variables is not strong. The adjusted r-squared value shows that a moderate amount of the variance is explained by this model. While you could use this model, I think it would be better to use a model that includes different variables for better results.
...
Yards Gained seems to not be impactful at all when it comes to wins, the least impactful in fact. The best model I tried includes all factors but excludes Yards Gained. Points scored seems to have the biggest impact on wins, as including it with the other variables, with the exception of Yards Gained, improves the model.
...
This model can be used to understand how different aspects of football performance impact the number of wins. So I can look at an NFL team and see how often the teams with the most points scored end up winning the most games. Ultimately, I can look at a team's pattern of actions like Takeaways or Giveaways and predict how their season will go/end. This can be useful when covering teams, and thus, predicting who will win a game between two teams.
